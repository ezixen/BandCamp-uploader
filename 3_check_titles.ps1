...
foreach ($folder in $resolved.Folders) {
  ...
  try {
    ...
    $msg = "CHECK FAILED: $folder — $($_.Exception.Message)"
    $failed.Add($msg) | Out-Null
    Write-Host "ERROR: $msg" -ForegroundColor Red
  }
}
...
if ($resolved.Folders.Count -eq 0) {
  Write-Host "Nothing to check — fix paths and try again." -ForegroundColor Red
  exit 1
}
...
if ($okFolders.Count -gt 0) {
  ...
  if ($okFolders.Count -eq 1) {
    Write-Host ("  .\4_bandcamp_uploader.ps1 `"{0}`"" -f $okFolders[0])
  } else {
    Write-Host ("  .\4_bandcamp_uploader.ps1 `"{0}`"" -f ($okFolders -join '; '))
  }
}
