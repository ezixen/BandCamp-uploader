# 4) Upload album draft(s) to Bandcamp (does not publish)
# Supports one path, or several separated by ; (or , between drive paths)
#
#   .\4_bandcamp_uploader.ps1
#   .\4_bandcamp_uploader.ps1 "d:\music\a; d:\music\b"
...
try {
  ...
  Write-Host "ERROR: Chrome CDP not on 9222. Run .\2_start_chrome.ps1 and log into Bandcamp first." -ForegroundColor Red
  exit 1
}
...
foreach ($folder in $resolved.Folders) {
  ...
  try {
    ...
    $msg = "UPLOAD FAILED: $folder — $($_.Exception.Message)"
    $failed.Add($msg) | Out-Null
    Write-Host "ERROR: $msg" -ForegroundColor Red
  }
}
